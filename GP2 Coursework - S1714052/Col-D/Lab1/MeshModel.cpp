...
void MeshModel::init(Vertex* vertices, unsigned int numVertices, unsigned int* indices, unsigned int numIndices)
{
	IndexedModel model;
...
void MeshModel::initModel(const IndexedModel& model)
{
...
void MeshModel::updateSphereData(glm::vec3 pos, float radius)
{
	meshSphere.SetPos(pos);
	meshSphere.SetRadius(radius);
}

void MeshModel::Move(int dist)
{
